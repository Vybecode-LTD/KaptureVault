...
- **Test status (T-16 CLOSED):** `Avalonia.Headless.XUnit` smoke tests, the CI test job (`.github/workflows/tests.yml`), and the `dotnet format --verify` + `dotnet list --vulnerable` gates are all **live and green**. The backend repo has its own vitest + `tsc` CI. Client suite **130**, backend **59**. Only remaining nicety: coverage-% isn't tracked yet (CI already emits Cobertura).
