...
All managed docs share **one `version`** (currently **1.13.0**) and carry YAML frontmatter. App version (`1.0.7` (shipped 2026-06-01)) is tracked separately via `app-version`. *(There is also a non-managed design reference, `docs/F-02-online-vault-design.md`, with no shared version.)*
...
**F-02 Phase 3 — 🟡 IN PROGRESS (2026-06-01):** design approved (`docs/F-02-PHASE-3-DESIGN.md` — multi-object screenshot sync + web-unlock meta + multi-object quota; decisions: PNG, **require a vault password**, oldest-first over-quota, client-precheck + server-prefix-sum). **Slice A done** (`3b5c131`): KDF params in `vault.db.meta` v2 for web-vault key derivation; suite 123→124. **Slice B (encryption interlock) is next.** Constraint discovered: screenshots are plaintext `.bmp` on disk → must be encrypted client-side before upload (needs a binary AES-GCM path, slice C).
