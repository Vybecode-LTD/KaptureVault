...
**F-02 Phase 3 — 🟡 IN PROGRESS (2026-06-01):** design approved (`docs/F-02-PHASE-3-DESIGN.md` — multi-object screenshot sync + web-unlock meta + multi-object quota; decisions: PNG, **require a vault password**, oldest-first over-quota, client-precheck + server-prefix-sum; 8 slices A–H). **Slices A–E done + CI-green:** **A** (`3b5c131`) KDF params (salt/iterations/kdf) in `vault.db.meta` v2 for web-vault key derivation; **B** (`c716d20`) **encryption interlock** — require a vault password to enable the Online Vault (`R2StorageProvider` refuses upload when `!IsActive` = authoritative backstop; `OnlineAccountViewModel` gates on `IEncryptionService.IsActive` + a `VaultPasswordRequired` flag + the "sole key" panel warning; also corrected the now-stale `IsPaid` sync-gate → free-sync-needs-encryption per Phase 2); **C** (`912821a`) binary `EncryptBytes`/`DecryptBytes` (AES-GCM raw bytes, no `ENC:` prefix; tamper/wrong-key/not-active → throw); **backend D** (`0193551`) `/vault/object/{put,get,delete}-url` + `GET /vault/objects` (keys validated to `screenshots/<name>`); **backend E** (`6e4570c`) multi-object quota — the `PUT /vault/meta` commit now sums **all** vault objects via R2 list and rejects over-quota *without deleting* (client trims oldest-first). Client suite 124→**130**, backend 51→**59**. **Next: slice F** (client screenshot pipeline: enumerate non-expired → re-encode BMP→PNG (SkiaSharp) → `EncryptBytes` → upload via the object API, incremental sync-state, oldest-first on a 413, orphan cleanup), then **G** (restore: download+decrypt+write, resolve-by-filename) + **H** (UX + docs). Key constraint: screenshots are plaintext `.bmp` on disk and the DB stores a device-local *path* → encrypt before upload + key the sync by *filename*.
