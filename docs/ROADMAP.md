...
| 3 | Client vault-sync v2 — multi-object sync (`vault.db` + re-encoded screenshot images), quota-aware; carry salt/KDF in `vault.db.meta` for web unlock | 🟡 **IN PROGRESS** — design approved (`docs/F-02-PHASE-3-DESIGN.md`); **slice A done** (KDF in meta, `3b5c131`); slices B–H remain |
