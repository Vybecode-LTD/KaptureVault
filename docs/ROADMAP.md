...
| 3 | Client vault-sync v2 — multi-object sync (`vault.db` + re-encoded screenshot images), quota-aware; carry salt/KDF in `vault.db.meta` for web unlock | 🟡 **IN PROGRESS** — design approved (`docs/F-02-PHASE-3-DESIGN.md`, 8 slices A–H). ✅ **A–E done + CI-green** (KDF meta · encryption interlock · binary crypto · backend object API · multi-object quota); ⬜ **F** (screenshot pipeline — NEXT), **G** (restore), **H** (UX+docs). Client **130** / backend **59**. See the slice tracker ↓ |
...
- [ ] **F — client screenshot sync pipeline (NEXT, the big one):** enumerate non-expired screenshots → re-encode BMP→PNG (SkiaSharp) → `EncryptBytes` → upload via the object API; incremental `online_sync_state.json` (skip unchanged); oldest-first trim on a `413`; orphan cleanup of deleted/expired screenshots.
- [ ] **G — restore:** `GET /vault/objects` → download each → `DecryptBytes` → write the PNG into the local `screenshots/` dir; resolve-by-filename (the DB stores device-local paths, so a restored DB's screenshot paths must be re-pointed).
- [ ] **H — UX + docs/release:** panel status ("syncing screenshots…", "N not synced — over quota"); reconcile docs; consider cutting **v1.0.8** once F/G land + a fresh end-to-end smoke passes.
