...
**Test project: LIVE** (`KaptureVault.Tests`, xUnit + NSubstitute + FluentAssertions + coverlet + `Avalonia.Headless.XUnit`, on `KaptureVault.slnx`). **207 tests passing** as of 2026-06-02 (**+25 Phase 6 file hosting** — `FileHostingServiceTests` 9 [upload/encrypt/download/list/delete/share], `FileHostingViewModelTests` 8 [folder grouping/navigation/upload-folder/share-gating], `KaptureOnlineApiClientTests` +5 [file API], `ServiceRegistrationTests` +1 [`IFileHostingService`]; backend gained `files`/`shares` suites — see below); 182 through Phase 4 + P5 (**+14 "P5"** — `CloudSyncManagerTests` +9 [decouple routing/no-op], `OnlineAccountViewModelTests` net +0 [dropped use-for-sync, added ShowLogin/SyncNow/IsSyncingAutomatically/handoff], `OnlineAccountServiceTests` +3 [handoff code], `KaptureOnlineApiClientTests` +1 [handoff], `ServiceRegistrationTests` net +0 [`IOnlineVaultSync` replaces `ISyncProviderController`]; 130 through Phase 3 A–E; **+22 Phase 3 F** — client object API +5, `SkiaScreenshotImageCodec` +3, `ScreenshotSyncService` upload pipeline +12, DI registration +2; **+10 Phase 3 G** — `ScreenshotSyncService` restore +6, `CaptureEntry` resolve-by-filename +4; **+6 Phase 4 desktop** — `OnlineAccountViewModel` use-for-sync / live provider switch +5, `ISyncProviderController` DI +1). *(The Phase 4 web vault is a separate repo — `Kapture.Tools-Website` — not covered by this suite; its inline script is syntax-checked via `node --check` and smoke-tested by the maintainer.)* Persistence seams in place: base-dir (`EncryptionService`), connection-string (`DatabaseService`). The app project excludes `KaptureVault.Tests/**` from its compile glob. The headless tier uses `TestAppBuilder` (`[assembly: AvaloniaTestApplication]`) over the real `App`.
...
| `Services/ServiceRegistrationTests` | KV-010/T-10 DI composition + F-02 online services registered + both cloud providers present; **Phase 3 F: `IScreenshotImageCodec` + `IScreenshotSyncService`; P5: `IOnlineVaultSync` registered** (`ISyncProviderController` retired) | 24 |
| `CloudSyncManagerTests` | **P5a decouple:** `SyncDriveNowAsync`/`SyncOnlineVaultNowAsync` route by ProviderName + no-op (return false, no file I/O) when the provider is absent/unauthenticated; cross-provider isolation; `SyncAsync(null)` throws; Providers map; start-both-timers-then-Dispose | 9 |
...
| `Services/CloudSync/OnlineAccountServiceTests` | F-02 P2: secret-less sign-in, DPAPI session + auto-refresh (near-expiry + 401), sign-out, entitlement from /me, **cached quota/used from /me**, checkout/portal URLs; **P5c: `CreateWebVaultHandoffCodeAsync` (signed-in / signed-out / API-fail)** | 17 |
...
| `ViewModels/OnlineAccountViewModelTests` | F-02: account-panel logic — subscribe/billing open URLs, sign-out, storage-summary; **Phase 3 B: vault-password gating** (`VaultPasswordRequired`); **P5: sign-in reports automatic sync (`IsSyncingAutomatically`), `ShowLogin`, `SyncNowCommand` (signed-in/out + no-password), and `OpenVault` appends the `#handoff=` fragment (or plain URL)** | 18 |
...
| `Services/CloudSync/FileHostingServiceTests` | **Phase 6:** upload pipeline (unencrypted PUT→commit, encrypt-before-PUT marks private, encrypt-without-password throws, 250 MB cap, R2-fail no-commit), download (decrypt vs as-is), list/delete/share/revoke delegation | 9 |
| `ViewModels/FileHostingViewModelTests` | **Phase 6D:** Files-manager logic — root grouping + top folders, navigate in/up, upload assigns current/null folder, new-folder visible under parent, share-encrypted blocked, share returns url, delete removes | 8 |

**Total: 207 tests.** *(Phase 6: +`FileHostingServiceTests` 9, +`FileHostingViewModelTests` 8, KaptureOnlineApiClientTests 17→22, ServiceRegistrationTests 24→25. P5: +`CloudSyncManagerTests` 9; OnlineAccountServiceTests 14→17; OnlineAccountViewModelTests → 18, the use-for-sync/`ISyncProviderController` surface replaced by handoff/`IOnlineVaultSync`.)*

> **F-02 Online Vault backend is a SEPARATE repo** — `kapturevault-backend` (`C:\dev\kapturevault-backend` / `github.com/Vybecode-LTD/kapturevault-backend`) with its **own** test suite: **85 vitest tests + `tsc --noEmit` typecheck + GitHub Actions CI** (`npm ci` → typecheck → test). Phase 2 added `auth-tokens`/`free-vault`/`quota`/`vault-quota`/`cors`/`me-tier` (26 → 51); Phase 3 added `vault-objects` + extended `vault-quota` (51 → 59); P5c added `handoff` (59 → 65); **Phase 6 added `files` (gates, put-url cap/quota, commit HEAD/over-cap-delete, get-url ownership, delete re-bank, encrypted+folder, shared-quota) + `shares` (free-blocked, public-download 302, can't-share-others, unknown/revoked/expired 404, revoke auth+ownership, encrypted→409, cascade) (65 → 85)**. Those are **not** part of the .NET `KaptureVault.Tests` count above.
