...
**Total: 182 tests.** *(P5: +`CloudSyncManagerTests` 9; KaptureOnlineApiClientTests 16→17, OnlineAccountServiceTests 14→17; OnlineAccountViewModelTests 19→18 and ServiceRegistrationTests 24→24 net-flat as the use-for-sync/`ISyncProviderController` surface was replaced by handoff/`IOnlineVaultSync`.)*

> **F-02 Online Vault backend is a SEPARATE repo** — `kapturevault-backend` (`C:\dev\kapturevault-backend` / `github.com/Vybecode-LTD/kapturevault-backend`) with its **own** test suite: **65 vitest tests + `tsc --noEmit` typecheck + GitHub Actions CI** (`npm ci` → typecheck → test). Phase 2 added `auth-tokens`/`free-vault`/`quota`/`vault-quota`/`cors`/`me-tier` (26 → 51); **Phase 3 added `vault-objects` + extended `vault-quota` (51 → 59); P5c added `handoff` (create-requires-auth, round-trip, single-use, expiry, bad/missing code, refresh-not-a-bearer) (59 → 65)**. Those are **not** part of the .NET `KaptureVault.Tests` count above.
