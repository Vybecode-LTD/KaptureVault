...
| `Services/ServiceRegistrationTests` | KV-010/T-10 DI composition + F-02 online services registered + both cloud providers present; **Phase 3 F: `IScreenshotImageCodec` + `IScreenshotSyncService`; Phase 4: `ISyncProviderController` registered** | 24 |
...
| `ViewModels/OnlineAccountViewModelTests` | F-02 P2: Settings account-panel logic — subscribe/billing open URLs, sign-out clears provider, storage-summary formatting; **Phase 3 B: enabling requires a vault password**; **Phase 4: "use for sync" switches the live provider + persists, blocked without a password, IsSyncTarget/CanUseForSync, sign-out clears the live provider** | 19 |
...
**Total: 168 tests.** *(Phase 3 F/G client suites: ScreenshotSyncServiceTests 18, SkiaScreenshotImageCodecTests 3, CaptureEntryTests 4; KaptureOnlineApiClientTests 11→16, ServiceRegistrationTests 21→24; Phase 4: OnlineAccountViewModelTests 14→19, +`ISyncProviderController` DI.)*
