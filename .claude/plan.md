...
All managed docs share one `version` (currently **1.14.0**) with YAML frontmatter.
...
## Current state (2026-06-01) — see `docs/HANDOFF.md` for live detail
- **Shipped release: v1.0.7.** Client tests **130**, backend **59**; both repos green, pushed, CI green.
...
- **Current initiative — F-02 "Online Vault"** (free encrypted cloud sync; paid file hosting + share links):
  ...
  - **Phase 3 (client vault-sync v2) in progress** — slices A–E done (KDF-in-meta, encryption interlock, binary crypto, backend object API, multi-object quota). **Next: slice F** — the client screenshot sync pipeline (re-encode → encrypt → upload). Tracker: `docs/F-02-PHASE-3-DESIGN.md` § 11.
